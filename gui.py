...
import constants as vconf_settings
...
class SplashScreen(tk.Toplevel):
    ...
    def init_ui(self):
        ...
        label_text_3 = tk.Label(text_frame, text="University at Buffalo's Aga Lab 2024", font=("Helvetica", 10), bg="white")
...
class GUI(tk.Tk):
    def __init__(self):
        super().__init__()
        global app
        app = self  # Set the global reference to the current instance
        ...
        self.ssh_connected = False  # Add this line to track SSH connection status
        ...
        # self.debug_button_console = None
        ...
        self.connect_to_server()  # Attempt to connect to the SSH server

        # Initialize script_process attribute
        self.script_process = None

        # Initialize script_row_counter attribute
        self.script_row_counter = 0

        # Initialize check_remote_directory_var attribute
        self.check_remote_directory_var = tk.BooleanVar()  # Add this line

        # Determine the base path and icon path
        if getattr(sys, 'frozen', False):
            # Running in a bundle
            base_path = os.path.join(
                sys._MEIPASS)  # noqa: Access to protected member _MEIPASS is necessary for PyInstaller
        ...
        try:
            ...
            print(f"Failed to set icon: {e}")  # Optionally handle the exception with a debug message

        # Initialize paths to settings files and other scripts
        self.constants_last_path = os.path.join(base_path, 'constants_last.txt')
        self.constants_path = os.path.join(base_path, 'constants.py')
        self.default_settings_path = os.path.join(base_path, 'default_settings.txt')
        self.main_path = os.path.join(base_path, 'main.py')
        self.gzip_and_unzip_path = os.path.join(base_path, 'gzip_and_unzip.py')
        self.check_remote_directory_path = os.path.join(base_path, 'check_remote_directory.py')
        self.clean_up_molecule_list_path = os.path.join(base_path, 'clean_up_molecule_list.py')
        self.generate_conformers_vconf_path = os.path.join(base_path, 'generate_conformers_vconf.py')
        self.cmdline_TMoleX_process_path = os.path.join(base_path, 'cmdline_TMoleX_process.py')
        self.submit_remote_jobs_to_cluster_path = os.path.join(base_path, 'submit_remote_jobs_to_cluster.py')
        self.check_cluster_queue_path = os.path.join(base_path, 'check_cluster_queue.py')
        self.grab_files_from_cluster_path = os.path.join(base_path, 'grab_files_from_cluster.py')
        self.write_new_inp_file_path = os.path.join(base_path, 'write_new_inp_file.py')
        self.dictionary_path = os.path.join(base_path, 'dictionary.py')
        ...
        # Load settings from constants_last_path
        ...
        # Set up GUI sections
        self.setup_script_settings_tab()
        self.create_console_tab()
        self.create_vconf_settings_section()
    ...
    def redirect_stdout_stderr(self):
        sys.stdout = ConsoleStream(self.append_to_console)
        sys.stderr = ConsoleStream(self.append_to_console)
    ...
    def on_mouse_wheel_script_settings(self, event):
        ...
        elif current_tab == str(self.file_editor_tab):
            self.file_editor_canvas.yview_scroll(int(-1 * (event.delta / 120)), "units")

    def run_script(self):
        # Reset the termination trigger before running the script
        ...
        self.script_process = Thread(target=self.execute_script, daemon=True)
    ...
    def execute_script(self):
        ...
        try:
            ...
            if getattr(sys, 'frozen', False):
                # Running in a bundle
                bundle_dir = sys._MEIPASS  # noqa: Access to protected member _MEIPASS is necessary for PyInstaller
                ...
                script_path = os.path.join(script_directory, 'main.py')  # Construct the path to main.py

                self.append_to_console(f"Executing script: {script_path}")  # Debug statement
                ...
                self.main_pid = self.process.pid  # Store the PID of the main.py process
                self.append_to_console(f"Started process with PID: {self.main_pid}")  # Debug statement

                # Capture and log stdout
                def capture_stdout(pipe):
                    try:
                        for line in iter(pipe.readline, ''):
                            if closing_flag:  # Check if the GUI is closing
                                break
                            self.append_to_console(line.strip())
                        pipe.close()
                    except Exception as stdout_exception:
                        self.append_to_console(f"Stdout capture error: {stdout_exception}")
                ...
                def capture_stderr(pipe):
                    try:
                        ...
                        pipe.close()
                    except Exception as stderr_exception:
                        self.append_to_console(f"Stderr capture error: {stderr_exception}")

                stdout_thread = Thread(target=capture_stdout, args=(self.process.stdout,))
                stderr_thread = Thread(target=capture_stderr, args=(self.process.stderr,))
                stdout_thread.start()
                stderr_thread.start()
                ...
                self.append_to_console("Script execution completed.")  # Debug statement
        except Exception as exec_exception:
            self.append_to_console(f"An error occurred: {exec_exception}")

    def append_to_console(self, text):
        if closing_flag:
            return  # Avoid updating the console if the application is closing
    ...
    def setup_script_settings_tab(self):
        ...
        self.terminate_button.grid(row=2, column=0, pady=10)
    ...
    def create_console_tab(self):
        ...
        self.terminate_button_console = ttk.Button(self.console_tab, text="Terminate Script",
                                                   command=self.terminate_script)
        self.terminate_button_console.pack(pady=5)

        # Debug button to manually append text to the console
        # self.debug_button_console = ttk.Button(self.console_tab, text="Debug Console",
                                               # command=lambda: self.append_to_console("Debugging console output"))
        # self.debug_button_console.pack(pady=5)
    ...
    def create_file_editor_section(self):
        ...
        self.file_editor_canvas = tk.Canvas(self.file_editor_tab)
        file_editor_scrollbar = ttk.Scrollbar(self.file_editor_tab, orient="vertical",
                                              command=self.file_editor_canvas.yview)
        file_editor_scrollable_frame = ttk.Frame(self.file_editor_canvas)

        file_editor_scrollable_frame.bind(
            "<Configure>",
            lambda e: self.file_editor_canvas.configure(
                scrollregion=self.file_editor_canvas.bbox("all")
            )
        )

        self.file_editor_canvas.create_window((0, 0), window=file_editor_scrollable_frame, anchor="nw")
        self.file_editor_canvas.configure(yscrollcommand=file_editor_scrollbar.set)

        self.file_editor_canvas.pack(side="left", fill="both", expand=True)
    ...
    def connect_to_server(self):
        ...
        try:
            ...
            self.ssh_connected = True  # Set to True if connection is successful
        except Exception as e:
            self.ssh_connected = False  # Set to False if connection fails
            if hasattr(self, 'console_text') and self.console_text:
                self.append_to_console(f"Failed to connect to server: {e}")  # Append error message to the console
            else:
    ...
    def terminate_script(self):
        ...
        # Additional logic to terminate any running processes
        if self.process and self.process.poll() is None:
            try:
                self.process.terminate()
                self.process.wait(timeout=5)  # Wait up to 5 seconds for the process to terminate
        ...
        # Terminate any other Python processes if needed
        current_pid = os.getpid()
        self.append_to_console(f"Current process ID: {current_pid}")

        for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
            # self.append_to_console(f"Checking process: {proc.info}")

            if proc.info['pid'] != current_pid and 'python' in proc.info['name'].lower():
                if getattr(sys, 'frozen', False):
                    # Additional checks for frozen state
                    if 'main' in ' '.join(proc.info['cmdline']):
                        try:
                            proc.terminate()
                    ...
                    # Regular checks for non-frozen state
                    try:
                        proc.terminate()
                        proc.wait(timeout=5)  # Wait up to 5 seconds for the process to terminate
                        self.append_to_console(f"Terminated process: {proc.info['pid']}")
    ...
    def create_script_editor(self, parent, script_name, script_path, column):
        ...
        script_directory = os.path.dirname(os.path.abspath(__file__))  # Get the directory of the current script
        full_script_path = os.path.join(script_directory, script_path)  # Construct the path to the script

        self.script_texts[full_script_path] = text_widget

        # Load the initial content of the script
        self.load_file_content(full_script_path, text_widget)
    ...
    def on_closing(self):
        global closing_flag
        closing_flag = True  # Set the flag to indicate the GUI is closing
        self.save_settings()
        ...
        self.append_to_console("Script termination process completed.")
        self.destroy()
        os._exit(0)  # Forcefully exit the entire script

def run_main_gui():
    app = GUI()
    app.redirect_stdout_stderr()  # Add this line to redirect stdout/stderr
    app.mainloop()
...
if __name__ == "__main__":
    ...
    show_splash_screen(root, splash_img_path)
