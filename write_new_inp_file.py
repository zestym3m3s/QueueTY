...
from constants import (
    list_folder, list_folder_name, template_name,
    extract_cosmo_files_to_cosmo_folder, write_inp_file,
...
def generate_inp_file(cosmo_files, target_directory):
    ...
    list_folder = os.path.dirname(os.path.dirname(settings['SDF_FILENAME']))
    inp_file_path = os.path.join(list_folder, f"{template_name}_{constants.list_folder_name}.inp")
...
def main():
    if not write_new_inp_file_script:
    ...
    if extract_cosmo_files_to_cosmo_folder:
    ...
    print(f"\nextract_cosmo_files_to_cosmo_folder is {'enabled' if extract_cosmo_files_to_cosmo_folder else 'disabled'}")
    print(f"write_inp_file is {'enabled' if write_inp_file else 'disabled'}")
